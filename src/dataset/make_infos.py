...
from supervisely.project.pointcloud_episode_project import EpisodeItemInfo
from supervisely.project.pointcloud_project import PointcloudItemInfo
...
def collect_instances(
    ann_frame: Union[sly.PointcloudAnnotation, sly.PointcloudEpisodeFrame],
    class2id: dict,
    cv_task: str,
):
    instances = []
...
def from_splits(
    ...
    for dataset in project.datasets:
        ...
        names = [s[1] for s in split_names if dataset.name == s[0]]
        for name in names:
            # Collect instances
            if is_episodes:
